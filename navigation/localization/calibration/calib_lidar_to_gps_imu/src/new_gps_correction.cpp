...
struct ceres_get_relative_position {
  ...
  bool operator()(const T *q, const T *t, T *residual) const {
    ...
    Eigen::Quaternion<T> q_incre{q[3], (T)0, (T)0, q[2]};
    Eigen::Matrix<T, 3, 1> t_incre{t[0], t[1], (T)0.0};
    // q_incre = q_incre.
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d _odom_t,
    ...
    return (
        new ceres::AutoDiffCostFunction<ceres_get_relative_position, 2, 4, 3>(
...
int main(int argc, char **argv) {
  ...
  Eigen::Vector3d tran_90_angle(3.1415926 / 2.0, 0, 0);
  Eigen::Quaterniond tran_90_quater =
      Eigen::AngleAxisd(tran_90_angle[0], Eigen::Vector3d::UnitZ()) *
      Eigen::AngleAxisd(tran_90_angle[1], Eigen::Vector3d::UnitY()) *
      Eigen::AngleAxisd(tran_90_angle[2], Eigen::Vector3d::UnitX());
  ...
  tf::Quaternion quat_tf(m_q_w_incre.x(), m_q_w_incre.y(), m_q_w_incre.z(),
                         m_q_w_incre.w());
  double roll, pitch, yaw;
  tf::Matrix3x3(quat_tf).getRPY(roll, pitch, yaw);
  std::cout << "livox_to_gps angle: " << roll * 57.3 << " " << pitch * 57.3
            << " " << yaw * 57.3 << std::endl;
  ...
  // Eigen::Vector3d tran_90_angle1(1.0/57.3, 0, 0);
  // Eigen::Quaterniond tran_90_quater1 =
  //     Eigen::AngleAxisd(tran_90_angle1[0], Eigen::Vector3d::UnitZ()) *
  //     Eigen::AngleAxisd(tran_90_angle1[1], Eigen::Vector3d::UnitY()) *
  //     Eigen::AngleAxisd(tran_90_angle1[2], Eigen::Vector3d::UnitX());
  // m_q_w_incre = tran_90_quater1;
