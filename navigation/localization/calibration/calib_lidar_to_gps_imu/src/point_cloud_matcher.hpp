...
class PointCloudMatcher {
 public:
  ...
  void GetPL_ICP(pcl::PointCloud<PointType>::Ptr m_laser_cloud_corner_last,
    ...
    // pcl::PointCloud<PointType>() ); m_t_w_curr = m_q_w_last * m_t_w_incre +
    // m_t_w_last; m_q_w_curr = m_q_w_last * m_q_w_incre;
    ...
    for (int iterCount = 0; iterCount < m_para_icp_max_iterations;
      ...
      // m_para_buffer_incremental + 4, covariance_yy);
    ...
    // covariance_xx[10], covariance_xx[15]);
    // // covariance_t = Eigen::Vector3d(covariance_yy[0], covariance_yy[4],
    // covariance_yy[8]); odom_covariance[0] = covariance_yy[0];
