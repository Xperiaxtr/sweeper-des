...
public:
    ...
    PointCloudMatcher()
    {
        ...
        m_down_sample_filter_intensity.setLeafSize(0.3, 0.3, 0.3);
        ...
        m_laser_cloud_intensity_from_map = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());

        m_kdtree_corner_from_map = pcl::KdTreeFLANN<PointType>::Ptr(new pcl::KdTreeFLANN<PointType>());
        m_kdtree_intensity_from_map = pcl::KdTreeFLANN<PointType>::Ptr(new pcl::KdTreeFLANN<PointType>());
        ...
        laser_cloud_intensity_stack = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
    ...
    // void GetPlicpTran(Eigen::Quaterniond &tran_quater, Eigen::Vector3d &tran_pos)
    // {
    //     tran_quater = m_q_w_incre;
    //     tran_pos = m_t_w_incre;
    //     // m_t_w_incre = Eigen::Vector3d(0.0, 0.0, 0.0);
    //     reset_incremtal_parameter();
    // }

    void GetPL_ICP(pcl::PointCloud<PointType>::Ptr m_laser_cloud_corner_last, pcl::PointCloud<PointType>::Ptr m_laser_cloud_surf_last, pcl::PointCloud<PointType>::Ptr m_laser_cloud_intensity_last)
    {
        ...
        m_kdtree_intensity_from_map->setInputCloud(m_laser_cloud_intensity_from_map);
        ...
        for (int iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++)
        {
            ...
            for(int i = 0; i < laser_intensity_pt_num; i++)
            {
                ...
                pointAssociateToMap(&pointOri, &pointSel, refine_blur(pointOri.intensity, m_minimum_pt_time_stamp, m_maximum_pt_time_stamp), if_undistore_in_matching);
                m_kdtree_intensity_from_map->nearestKSearch(pointSel, intensity_search_num, m_point_search_Idx, m_point_search_sq_dis);
                if (m_point_search_sq_dis[intensity_search_num - 1] < 2.0)
                {
                    ...
                    if (IF_INTENSITY_FEATURE_CHECK)
                    {
                        ...
                        {
                            Eigen::Vector3d tmp(m_laser_cloud_intensity_from_map->points[m_point_search_Idx[j]].x,
                                                m_laser_cloud_intensity_from_map->points[m_point_search_Idx[j]].y,
                                                m_laser_cloud_intensity_from_map->points[m_point_search_Idx[j]].z);
                            center = center + tmp;
                    ...
                    if (line_is_avail)
                    {
                        if (ICP_LINE)
                        {
                            ...
                            {
                                cost_function = ceres_icp_point2line<double>::Create(curr_point,
                                                                                     pcl_pt_to_eigend(m_laser_cloud_intensity_from_map->points[m_point_search_Idx[0]]),
                                                                                     pcl_pt_to_eigend(m_laser_cloud_intensity_from_map->points[m_point_search_Idx[1]]),
                                                                                     refine_blur(pointOri.intensity, m_minimum_pt_time_stamp, m_maximum_pt_time_stamp) * 1.0,
                                                                                     Eigen::Matrix<double, 4, 1>(m_q_w_last.w(), m_q_w_last.x(), m_q_w_last.y(), m_q_w_last.z()),
                                                                                     m_t_w_last); //pointOri.intensity );
                            }
        ...
        std::cout << "corner_avail_num " << corner_avail_num << "surf_avail_num " << surf_avail_num << std::endl;
