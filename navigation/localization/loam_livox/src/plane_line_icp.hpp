...
class Plane_Line_ICP
{
  ...
  ros::Publisher o_pub_all_cloud;
    ...
    int   m_para_cere_max_iterations = 10;
    ...
    Plane_Line_ICP()
    {
        ...
        laserCloudCornerStack =pcl::PointCloud<PointType>::Ptr( new pcl::PointCloud<PointType>() );
        laserCloudSurfStack =pcl::PointCloud<PointType>::Ptr( new pcl::PointCloud<PointType>() );
    ...
    void GetCornerSurfMap(pcl::PointCloud<PointType> laserCloudMap)
    {
        for(size_t i = 0; i < laserCloudMap.points.size(); i++)
        {
            ...
            else    m_laser_cloud_surf_from_map->points.push_back(point_now);
        }

    }
    void ReceivePosition(Eigen::Quaterniond cin_gps_quaterniond, Eigen::Vector3d cin_gps_position)
    {
    ...
    void GetPL_ICP( pcl::PointCloud<PointType>::Ptr m_laser_cloud_corner_last, pcl::PointCloud<PointType>::Ptr m_laser_cloud_surf_last)
    {
        ...
        printf( "map corner num %d  surf num %d \n", laser_corner_pt_num, laser_surface_pt_num );
        ...
        for ( int iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++ )
        {
            ...
            for ( int i = 0; i < laser_corner_pt_num; i++ )
            {
                ...
                if ( m_point_search_sq_dis[ line_search_num - 1 ] < 2.0 )
                {
                    ...
                    if ( IF_LINE_FEATURE_CHECK )
                    {
                        ...
                        // std::cout<<"corner  saes"<<saes.eigenvalues()[ 0 ]<<"  "<<saes.eigenvalues()[ 1 ]<<"  "<<saes.eigenvalues()[ 2 ]<<std::endl;
            ...
            for ( int i = 0; i < laser_surface_pt_num; i++ )
            {
                ...
                if ( m_point_search_sq_dis[ plane_search_num - 1 ] < 10.0 )
                {
                    ...
                    if ( IF_PLANE_FEATURE_CHECK )
                    {
                        ...
                        {
                            Eigen::Vector3d tmp( m_laser_cloud_surf_from_map->points[ m_point_search_Idx[ j ] ].x,
                        ...
                        Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes( covMat );
                        // std::cout<<"surf  saes"<<saes.eigenvalues()[ 0 ]<<"  "<<saes.eigenvalues()[ 1 ]<<"  "<<saes.eigenvalues()[ 2 ]<<std::endl;
            ...
            for ( size_t ii = 0; ii < 1; ii++ )
            {
                ...
                if ( 1 )
                {
                    ...
                    {
                        if ( ( fabs( residuals[ 3 * i + 0 ] ) + fabs( residuals[ 3 * i + 1 ] ) + fabs( residuals[ 3 * i + 2 ] ) ) > std::min( 0.1, 5 * avr_cost ) ) // std::min( 1.0, 10 * avr_cost )
                        {
            ...
            covariance_blocks.push_back(make_pair(m_para_buffer_incremental, m_para_buffer_incremental));
            covariance_blocks.push_back(make_pair(m_para_buffer_incremental +4, m_para_buffer_incremental +4));
            // covariance_blocks.push_back(make_pair(m_para_buffer_incremental, m_para_buffer_incremental +4));
            ...
            covariance.GetCovarianceBlock(m_para_buffer_incremental, m_para_buffer_incremental, covariance_yy);
            covariance.GetCovarianceBlock(m_para_buffer_incremental + 4, m_para_buffer_incremental + 4, covariance_yy);
            ...
            // double delta_t = sqrt(m_t_w_incre.x() * m_t_w_incre.x() + m_t_w_incre.y() * m_t_w_incre.y() + m_t_w_incre.z() * m_t_w_incre.z());
            // double delta_r = sqrt(eulerAngle(0) * eulerAngle(0) + eulerAngle(1) * eulerAngle(1) + eulerAngle(2) * eulerAngle(2));
            // std::cout<<"delta T "<<delta_t<<" delta R "<<delta_r<<std::endl; 
            // std::cout<<"icp num "<<iterCount<<std::endl;
            // if( delta_t < 0.005 && delta_r < 0.0001) break;
            // std::cout<<"icp num "<<m_para_icp_max_iterations<<std::endl;
            std::cout<<"avail : "<<corner_avail_num<<" "<<surf_avail_num<<" "<<surface_rejecetion_num<<std::endl;
            ROS_INFO("coner_ration , surf_avail_ration, %d, %d, %d",corner_avail_num, surf_avail_num, surface_rejecetion_num);
            if(odom_cov_ < 0.0008) break;
        }
        // std::cout<<"t_now  "<<m_t_w_curr<<std::endl;
        // std::cout<<"q_now  "<<m_t_w_curr<<std::endl;
        // std::cout<<"corner_avail_num "<<corner_avail_num/corner_rejection_num<<"surf_avail_num "<<surf_avail_num/surf_rejecetion_num<<std::endl;
        // ROS_INFO("coner_ration , surf_avail_ration, %f, %f",corner_avail_num/(corner_rejection_num +corner_avail_num), surf_avail_num/(surface_rejecetion_num+surf_avail_num));
        ...
        m_q_w_last = m_q_w_curr;
        m_t_w_last = m_t_w_curr;
        // covariance_q = Eigen::Quaterniond(covariance_xx[0], covariance_xx[5], covariance_xx[10], covariance_xx[15]);
        // covariance_t = Eigen::Vector3d(covariance_yy[0], covariance_yy[4], covariance_yy[8]);
