...
struct Data_pair
{
    ...
    bool is_completed()
    {
        return (m_has_pc_corner && m_has_pc_surf && m_has_pc_full);
    }
    void reset_param()
    {
...
public:
    ...
    pcl::PointCloud<PointType>::Ptr last_full_cloud_;
    ...
    LaserMappingNew()
    {
        ...
        sub_full_cloud = nh.subscribe<sensor_msgs::PointCloud2>("/pc2_full", 10000, &LaserMappingNew::LaserCloudFullLastHandler, this);
        ...
        pub_full_cloud_ = nh.advertise<sensor_msgs::PointCloud2>("/pub_full_cloud", 1);
        pub_real_odom_ = nh.advertise<nav_msgs::Odometry>("/pub_now_odom", 1);
    ...
    void ResetParame()
    {
        ...
        last_full_cloud_.reset(new pcl::PointCloud<PointType>());
    ...
    {
        receive_cloud_now.add_pc_full(msg);
        if (receive_cloud_now.is_completed())
        {
            receive_clouds.push(receive_cloud_now);
            receive_cloud_now.reset_param();
    ...
    void GetPLIcp()
    {
        ...
        *plicp.m_laser_cloud_surf_from_map = *laserCloudSurfFromMapDS;
        ...
        // covariance_t = plicp.covariance_t;
    }
    ...
    void PublishRealtimeCloud()
    {
        // PointTypePose pose_6d_curr;
        // pose_6d_curr.q_w = m_q_w_curr.w();
        // pose_6d_curr.q_x = m_q_w_curr.x();
        // pose_6d_curr.q_y = m_q_w_curr.y();
        // pose_6d_curr.q_z = m_q_w_curr.z();
        // pose_6d_curr.x = m_t_w_curr.x();
        // pose_6d_curr.y = m_t_w_curr.y();
        // pose_6d_curr.z = m_t_w_curr.z();

        // pcl::PointCloud<PointType> full_cloud;
        // pointcloudAssociateToMap(*last_full_cloud_, full_cloud, pose_6d_curr);
        
        sensor_msgs::PointCloud2 cloud_msg;
        pcl::toROSMsg(*last_full_cloud_, cloud_msg);
        cloud_msg.header.stamp = ros::Time().fromSec(cloudrealtime);
        cloud_msg.header.frame_id = "/camera_init";
        pub_full_cloud_.publish(cloud_msg);
        ...
        pub_real_odom_.publish(odomAftMapped);
    ...
    void Run()
    {
        ...
        *last_full_cloud_ = receive_cloud_last.laser_cloud_full;
        timeLaserCloudSurfLast = receive_cloud_last.laser_time_surf;
        timeLaserCloudCornerLast = receive_cloud_last.laser_time_surf;

        if (/* newLaserCloudCornerLast && */ std::abs(timeLaserCloudCornerLast - timeLaserCloudSurfLast) < 0.005 /*  &&
            ...
            /*&& std::abs(timeLaserCloudSurfLast    - timeLaserOdometry) < 0.005*/)
        {
            ...
            // std::cout << std::setprecision(13) << "time  :  " << cloudrealtime << std::endl;
            // newLaserCloudCornerLast = false;
            // newLaserCloudSurfLast = false;
    ...
    void PerformGpsLoopClosure()
    {
        ...
        if ( (!saved_frist_gps_and_odom  && gps_correspond_mapping_covariance[0] < 0.1 && gps_correspond_mapping_covariance[1] < 0.1) || 
            (saved_frist_gps_and_odom < 6 && gps_correspond_mapping_covariance[0] < 0.05 && gps_correspond_mapping_covariance[1] < 0.05 &&
             (last_save_position.x() - mapping_odom_position.x()) * (last_save_position.x() - mapping_odom_position.x()) + (last_save_position.y() - mapping_odom_position.y()) * (last_save_position.y() - mapping_odom_position.y()) > 9.0))
        {
            saved_frist_gps_and_odom++;
            if(save_gps_correspond_odom_data)
            {
                std::ofstream myfile("../sweeper_ws/data/path/frist_gps_odom.txt", std::ios::app);
                myfile.clear();
                myfile << to_string(mapping_odom_position.x()) << '|' << to_string(mapping_odom_position.y()) << '|' << to_string(mapping_odom_position.z()) << '|'
                    << to_string(mapping_odom_quaterniond.x()) << '|' << to_string(mapping_odom_quaterniond.y()) << '|' << to_string(mapping_odom_quaterniond.z()) << '|'
                    << to_string(mapping_odom_quaterniond.w()) << '|' << to_string(key_point_time) << "\n"
    ...
    void PublishAndSaveMap()
    {
        ...
        for (size_t i = 0; i < globalMapKeyPosesDS->points.size(); i++)
        {
            ...
            *globalMapKeyFrames += surrounding_corner;
            *globalMapKeyFrames += surrounding_surf;
        }
        ...
        pcl::io::savePCDFileASCII("../sweeper_ws/src/sweeper_topworld/data/map/surround.pcd", *globalMapKeyFramesDS);
