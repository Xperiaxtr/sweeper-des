...
std::string Byte::double_to_hex(const uint32_t value) {
  uint8_t high_high = (value >> 24) & 0xFF;
  uint8_t high_low = (value >> 16) & 0xFF;
  uint8_t low_high = (value >> 8) & 0xFF;
  uint8_t low_low = value & 0xFF;
  std::string result = "";
  result += byte_to_hex(high_high);
  result += byte_to_hex(high_low);
  result += byte_to_hex(low_high);
  result += byte_to_hex(low_low);
  return result;
}
...
int8_t Byte::get_byte_signed(const int32_t start_pos,
  ...
  int32_t end_pos = std::min(start_pos + length - 1, BYTE_LENGTH - 1);
  int32_t real_len = end_pos + 1 - start_pos;
  int8_t result = *value_ >> start_pos;
  result &= RANG_MASK_1_L[real_len - 1];
  return result;
}
